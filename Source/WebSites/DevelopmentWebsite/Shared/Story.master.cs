    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (deviceManager.IsMobileDevice())
            {
                ...
                if (objSessionValue != null && objSessionValue.NoRedirection == false)
                {
                    if (Request.QueryString["TributeUrl"] != null)
                    {
                    ...
                    if (!(string.IsNullOrEmpty(mTrbUrl)) && !(string.IsNullOrEmpty(mTrbType)) && IsMobileViewOn &&
                        IsMobileRedirectOn && (mTrbType.ToLower().Equals("memorial")))
                    {
                        obTrb = objMisc.GetTributeUrlOnOldTributeUrl(obTrb, WebConfig.ApplicationType.ToString());

                        strStartUrl = string.Format("{0}{1}{2}{3}{4}{5}", "https://www.", WebConfig.TopLevelDomain,
                                                    "/mobile/index.html?tributeurl=", obTrb.TributeUrl, "&tributetype=",
                                                    mTrbType);
